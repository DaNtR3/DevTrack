...
const ModifyTeams = ({ roleID }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await api.modifyTeam({name, newname, description, usersToBeAssigned});

      if (result.success) {
        setSuccessMessage("¡La modificación del equipo ha sido existosa!"); // Set success message
      }
    } catch (err) {
      setError(
        err.response?.data?.message || "¡La modificación del equipo ha fallado"
      );
      console.error("Modify team error:", err);
    }
  };
